...
/** 一覧検索用の検索タグ TODO : flag-markみたいにuseStateを削除し今回はbuttonのonClickを呼ぶように修正 */
export function SearchTag({ title, isSelected } : { title: string, isSelected: boolean }) {
    ...
    return (
        <button className={`flex items-center justify-center ${selected ? 'bg-light-blue-100' : 'bg-white'} 
                ...
                text-base font-bold text-light-blue-1000 cursor-pointer`} onClick={() => setSelected(!selected) }>
            {title}
        </button>
    );
}
