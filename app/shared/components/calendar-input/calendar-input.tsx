...
export function CalendarInput(props: CalendarInputProps) {
  ...
  const [date, setDate] = useState<CalendarDate | null>(parseDate(todayDate.toString()));
  const [focusedDate, setFocusedDate] = useState(parseDate(todayDate.toString()));
  const { isShown, onChange, className } = props;
  // TODO : 年の選択肢が動的に表示されるように実装
  // let yearVal = todayDate.year - 5;
  // let japaneseYear = 2;
  // const yearOptions = Array(10).map(v => {
  //   const option = <option value={yearVal}>{yearVal}年(令和{japaneseYear}年)</option>; 
  //   yearVal++;
  //   return option;
  // });
    ...
    <Calendar className={`flex flex-col items-center w-max bg-white border-1 rounded-8 ${className ?? ''}`} value={date} onChange={changeDate} focusedValue={focusedDate} onFocusChange={setFocusedDate}>
      <div className='flex items-center gap-2 p-4'>
        ...
        <div className='flex items-center'>
          <Button type='button' className='!min-w-0 !size-11' slot='previous' size='sm' variant='outline' onClick={() => setFocusedDate(focusedDate.add({
          months: -1
        }))}>
          ...
          <Button type='button' className='!min-w-0 !size-11' slot='next' size='sm' variant='outline' onClick={() => setFocusedDate(focusedDate.add({
          months: 1
        }))}>
      ...
      <div className='flex self-stretch justify-between gap-4 p-4'>
        <Button type='button' variant='text' size='sm' onClick={() => changeDate(null)}>
          削除
        </Button>
